...
def search_customers(search_term):
    """Search customers using available data"""
    try:
        search_query = f"""
        SELECT 
            EMAIL_ID,
...
def main():
    ...
    with st.sidebar:
        ...
        if sentiment_filter and not email_data.empty:
...
def show_executive_dashboard(email_data, summary_stats):
    ...
    with col2:
        st.metric(
            ...
            delta=f"+{email_data['CUSTOMER_ID'].nunique() if not email_data.empty else 0} this period"
        )
    
    with col3:
        escalation_rate = (summary_stats['ESCALATIONS_NEEDED'] / summary_stats['TOTAL_EMAILS'] * 100) if summary_stats['TOTAL_EMAILS'] > 0 else 0
        st.metric(
            "🚨 Escalation Rate",
    ...
    with col4:
        positive_rate = (summary_stats['POSITIVE_SENTIMENT'] / summary_stats['TOTAL_EMAILS'] * 100) if summary_stats['TOTAL_EMAILS'] > 0 else 0
        st.metric(
            "😊 Positive Sentiment",
            f"{positive_rate:.1f}%",
    ...
    if not email_data.empty:
        ...
        for _, email in priority_emails.iterrows():
            ...
            <div class="{priority_class}">
                ...
                <em>{email['DATE_RECEIVED'].strftime('%Y-%m-%d %H:%M') if pd.notna(email['DATE_RECEIVED']) else 'N/A'}</em><br>
                {email['EXECUTIVE_SUMMARY'][:200] if pd.notna(email['EXECUTIVE_SUMMARY']) else 'No summary available'}...
            </div>
            """, unsafe_allow_html=True)
    else:
        st.info("📊 Connect to your Snowflake data tables to see live analytics")
...
def show_customer_search(email_data):
    ...
    if search_term and not email_data.empty:
        ...
        if not search_results.empty:
            ...
            for _, result in search_results.iterrows():
                ...
                with st.expander(f"📧 {result['CUSTOMER_NAME']} - {result['EMAIL_CLASSIFICATION']} ({result['DATE_RECEIVED'].strftime('%Y-%m-%d') if pd.notna(result['DATE_RECEIVED']) else 'N/A'})"):
        ...
        st.info("No data available for search. Connect your Snowflake tables.")
        ...
        with col2:
            if st.button("😞 Negative Sentiment"):
                ...
                if not negative.empty:
                    ...
                    st.info("No negative sentiment found")
        ...
        with col3:
            if st.button("📞 Follow-ups Required"):
                ...
                if not followups.empty:
                    ...
                    st.info("No follow-ups required")
        ...
        if selected_customer:
            ...
            with col1:
                ...
                <div class="customer-card">
                    ...
                    <p><strong>Date Range:</strong> {customer_emails['DATE_RECEIVED'].min().strftime('%Y-%m-%d') if pd.notna(customer_emails['DATE_RECEIVED'].min()) else 'N/A'} to {customer_emails['DATE_RECEIVED'].max().strftime('%Y-%m-%d') if pd.notna(customer_emails['DATE_RECEIVED'].max()) else 'N/A'}</p>
                </div>
                """, unsafe_allow_html=True)
            ...
            for _, email in recent_emails.iterrows():
                with st.expander(f"{email['EMAIL_CLASSIFICATION']} - {email['DATE_RECEIVED'].strftime('%Y-%m-%d %H:%M') if pd.notna(email['DATE_RECEIVED']) else 'N/A'}"):
                    col1, col2 = st.columns([3, 1])
                    
                    with col1:
                        st.write(f"**Summary:** {email['EXECUTIVE_SUMMARY'] if pd.notna(email['EXECUTIVE_SUMMARY']) else 'N/A'}")
                        if pd.notna(email['KEY_TOPICS_DISCUSSED']) and email['KEY_TOPICS_DISCUSSED']:
                            st.write(f"**Key Topics:** {email['KEY_TOPICS_DISCUSSED']}")
                        if pd.notna(email['NEXT_STEPS']) and email['NEXT_STEPS']:
                    ...
                    with col2:
                        ...
                        st.write(f"**Urgency:** {email['RESPONSE_URGENCY'] if pd.notna(email['RESPONSE_URGENCY']) else 'N/A'}")
                        st.write(f"**Escalation:** {'Yes' if email['ESCALATION_NEEDED'] else 'No'}")
    else:
        st.info("👤 Connect to your customer data to see individual profiles")
...
def show_analytics_deep_dive(email_data):
    ...
    if not email_data.empty:
        ...
        st.info("📈 Connect to your analytics data to see trends and insights")
...
def show_ai_insights(email_data):
    ...
    if not email_data.empty:
        ...
        **Customer Satisfaction Analysis:**
        - Total Communications Analyzed: {summary_data['total_emails']}
        - Average Sentiment Score: {summary_data['avg_sentiment']:.2f} (on scale of -1 to 1)
        - Escalation Rate: {summary_data['escalation_rate']:.1f}%
        - Urgent Responses Required: {summary_data['urgent_responses']}
        ...
        if at_risk_customers:
            ...
            for customer_id in at_risk_customers[:5]:  # Show top 5
                ...
                with st.expander(f"🚨 {customer_name} (ID: {customer_id})"):
                    ...
                    st.write(f"**Summary:** {latest_issue['EXECUTIVE_SUMMARY'] if pd.notna(latest_issue['EXECUTIVE_SUMMARY']) else 'N/A'}")
                    st.write(f"**Recommended Action:** {latest_issue['NEXT_STEPS'] if pd.notna(latest_issue['NEXT_STEPS']) else 'Contact customer immediately'}")
    else:
        st.info("🤖 Connect to your AI analytics data to see intelligent insights")
